...
class ConjGradSolver(utils.CommonSolver):
    @staticmethod
    def _matvec_functor(k, k_conj, eta, m, n):
        ...
        def inner(k, k_conj, eta, m, n, Av, Av2, Av2_x, Av2_y, v):
            v = v.reshape(m, n)
        ...
        matvec = lambda v: inner(k, k_conj, eta, m, n, Av, Av2, Av2_x, Av2_y, v)
        return matvec

    def __init__(self, k, p, J, y):
        super(ConjGradSolver, self).__init__(k, p, J, y)
    ...
    def benchmark(self, x_hat, eta):
        ...
        A = scipy.sparse.linalg.LinearOperator((m*n, m*n),
            self._matvec_functor(self.k, self.k_conj, eta, m, n))
